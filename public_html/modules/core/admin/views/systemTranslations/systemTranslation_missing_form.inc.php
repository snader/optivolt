...
<form method="POST" class="validateForm">
    ...
    <table class="sorted" style="width: 100%;">
        ...
        <tr class="topRow">
            <td colspan="3">
                <h2>
                    ...
                    <select class="required default" title="<?= _e(sysTranslations::get('sysTrans_enter_language_tooltip')) ?>" id="systemLanguageId" name="systemLanguageId">
                        ...
                        foreach (SystemLanguageManager::getLanguagesByFilter() AS $oLanguage) {
                            echo '<option ' . ($oLanguage->systemLanguageId == $oCurrentUser->systemLanguageId ? 'selected' : '') . ' value="' . $oLanguage->systemLanguageId . '">' . $oLanguage->name . '</option>';
                        }
                        ?>
                    </select>
                </h2>
        ...
        foreach ($aMissingLabels AS $sLabel) {
            ...
            echo '<td>' . $sLabel . '</td>';
            echo '<td>';
            echo '<input type="hidden" name="systemTranslations[labels][]" value="' . $sLabel . '" />';
            echo '<textarea name="systemTranslations[texts][]" style="width: 95%;"></textarea>';
            echo '</td>';
            echo '</tr>';
        }
